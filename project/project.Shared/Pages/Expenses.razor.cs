...
namespace project.Shared.Pages
{
    public partial class Expenses : ComponentBase
    {
        ...
        private string SortOption { get; set; } = "DateDesc";
        ...
        private bool IsFilterStaffPopupOpen { get; set; } = false;
        ...
        private ExpenditureItem? _itemToDelete;
        ...
        {
            { "Marketing", new List<SubCategoryItem> { new SubCategoryItem { Name = "Marketing and Advertising", Description = "Costs for ads, promotions, and online campaign" } } },
            ...
            { "Vendor", new List<SubCategoryItem> { new SubCategoryItem { Name = "Inventory Costs", Description = "Purchasing stock or raw materials for resale" } } },
        ...
        private IEnumerable<ExpenditureItem> FilteredExpenses
        {
            get
            {
                ...
                {
                    if (!string.IsNullOrEmpty(FilterCategory)) query = query.Where(e => e.Category == FilterCategory);
                    if (!string.IsNullOrEmpty(FilterStaff)) query = query.Where(e => e.Staff == FilterStaff);
                    if (!string.IsNullOrEmpty(FilterSearchQuery)) query = query.Where(e => e.InvoiceTitle.Contains(FilterSearchQuery, StringComparison.OrdinalIgnoreCase) || (e.Remark != null && e.Remark.Contains(FilterSearchQuery, StringComparison.OrdinalIgnoreCase)));
                }

                switch (SortOption)
                {
                    case "NameAsc": return query.OrderBy(e => e.InvoiceTitle);
                    case "NameDesc": return query.OrderByDescending(e => e.InvoiceTitle);
                    case "AmountAsc": return query.OrderBy(e => e.Amount);
                    case "AmountDesc": return query.OrderByDescending(e => e.Amount);
                    default: return query.OrderByDescending(e => e.Date);
                }
            }
        }
        ...
        private string GetIconForCategory(string category)
        {
            ...
            switch (category)
            {
                case "Marketing": path = "<polygon points='11 5 6 9 2 9 2 15 6 15 11 19 11 5'></polygon><path d='M19.07 4.93a10 10 0 0 1 0 14.14M15.54 8.46a5 5 0 0 1 0 7.07'></path>"; break;
                case "Staff": path = "<path d='M17 21v-2a4 4 0 0 0-4-4H5a4 4 0 0 0-4 4v2'></path><circle cx='9' cy='7' r='4'></circle><path d='M23 21v-2a4 4 0 0 0-3-3.87'></path><path d='M16 3.13a4 4 0 0 1 0 7.75'></path>"; break;
                case "Utility": path = "<polygon points='13 2 3 14 12 14 11 22 21 10 12 10 13 2'></polygon>"; break;
                case "Vendor": path = "<path d='M3 9l9-7 9 7v11a2 2 0 0 1-2 2H5a2 2 0 0 1-2-2z'></path><polyline points='9 22 9 12 15 12 15 22'></polyline>"; break;
                case "Finance": path = "<line x1='12' y1='1' x2='12' y2='23'></line><path d='M17 5H9.5a3.5 3.5 0 0 0 0 7h5a3.5 3.5 0 0 1 0 7H6'></path>"; break;
                case "Rental": path = "<rect x='4' y='2' width='16' height='20' rx='2' ry='2'></rect><line x1='9' y1='22' x2='9' y2='22'></line><line x1='15' y1='22' x2='15' y2='22'></line><line x1='12' y1='22' x2='12' y2='22'></line><line x1='12' y1='2' x2='12' y2='22'></line>"; break;
                case "Others": path = "<circle cx='12' cy='12' r='10'></circle><line x1='12' y1='8' x2='12' y2='12'></line><line x1='12' y1='16' x2='12.01' y2='16'></line>"; break;
        ...
        {
            NewExpense = new ExpenditureItem { Category = SelectedCategory == "All Expenses" ? Categories[0] : SelectedCategory, SubCategory = "", Date = new DateTime(SelectedYear, SelectedMonth, DateTime.Now.Day > 28 ? 1 : DateTime.Now.Day) };
            IsModalOpen = true;
        }

        private void CloseModal()
        {
            IsModalOpen = false; IsUploadPopupOpen = false; IsAmountPopupOpen = false; IsStaffPopupOpen = false; IsCategoryPopupOpen = false; IsSubCategoryPopupOpen = false; IsDownloadPopupOpen = false; IsFilterStaffPopupOpen = false; IsDeleteConfirmOpen = false;
        }
        ...
        private async Task HandleFileSelected(InputFileChangeEventArgs e)
        {
            ...
            var file = e.File;
            if (file != null) NewExpense.FileName = file.Name;
            await Task.CompletedTask;
        }

        private void OpenAmountPopup() { _tempAmountString = NewExpense.Amount == 0 ? "0" : NewExpense.Amount.ToString("0.##"); IsAmountPopupOpen = true; }
        private void AppendToAmount(string val) { if (_tempAmountString == "0" && val != ".") _tempAmountString = val; else { if (val == "." && _tempAmountString.Contains(".")) return; _tempAmountString += val; } }
        private void BackspaceAmount() { if (_tempAmountString.Length > 0) _tempAmountString = _tempAmountString.Substring(0, _tempAmountString.Length - 1); if (string.IsNullOrEmpty(_tempAmountString)) _tempAmountString = "0"; }
        private void ClearAmount() { _tempAmountString = "0"; }
        private void SaveAmount() { if (decimal.TryParse(_tempAmountString, out decimal result)) NewExpense.Amount = result; IsAmountPopupOpen = false; }

        private void OpenStaffPopup() { _staffSearchQuery = ""; IsStaffPopupOpen = true; }
        private void SelectStaff(string staffName) { NewExpense.Staff = staffName; IsStaffPopupOpen = false; }

        private void SelectCategoryFromPopup(string category)
        {
            if (NewExpense.Category != category) { NewExpense.Category = category; NewExpense.SubCategory = ""; }
            IsCategoryPopupOpen = false; IsSubCategoryPopupOpen = true;
        }

        private void OpenSubCategoryPopup() { IsSubCategoryPopupOpen = true; }
        private void SelectSubCategory(string subCategory) { NewExpense.SubCategory = subCategory; IsSubCategoryPopupOpen = false; }
        ...
        private async Task DownloadReport(string format)
        {
            if (format == "csv")
            {
                ...
                foreach (var item in FilteredExpenses) csv.AppendLine($"{EscapeCsv(item.Category)},{EscapeCsv(item.SubCategory)},{item.Date:yyyy-MM-dd},{EscapeCsv(item.Staff)},{EscapeCsv(item.InvoiceTitle)},{item.Amount},{EscapeCsv(item.Remark)}");
                var fileBytes = Encoding.UTF8.GetBytes(csv.ToString());
                await JS.InvokeVoidAsync("downloadFile", $"Expenses_{DateTime.Now:yyyyMMdd_HHmm}.csv", "text/csv", Convert.ToBase64String(fileBytes));
            }
            else if (format == "pdf") await JS.InvokeVoidAsync("window.print");
            IsDownloadPopupOpen = false;
        }

        private string EscapeCsv(string field) { if (string.IsNullOrEmpty(field)) return ""; if (field.Contains(",") || field.Contains("\"") || field.Contains("\n")) return $"\"{field.Replace("\"", "\"\"")}\""; return field; }

        private void OpenFilterStaffPopup() { _staffSearchQuery = ""; IsFilterStaffPopupOpen = true; }
        private void SelectStaffFilter(string staffName) { FilterStaff = staffName; IsFilterStaffPopupOpen = false; }

        private string GetCategoryTotal(string category)
        {
            var query = ExpensesDb.Where(e => e.Date.Month == SelectedMonth && e.Date.Year == SelectedYear);
            if (!string.IsNullOrEmpty(category)) query = query.Where(e => e.Category == category);
            return $"MYR {query.Sum(e => e.Amount):N2}";
        }

        private void GoToProfile() { Navigation.NavigateTo("/profile"); }

        private void RequestDelete(ExpenditureItem item) { _itemToDelete = item; IsDeleteConfirmOpen = true; }
        private void ConfirmDelete() { if (_itemToDelete != null) ExpensesDb.Remove(_itemToDelete); _itemToDelete = null; IsDeleteConfirmOpen = false; }
        ...
        {
            NewExpense = new ExpenditureItem { Id = item.Id, Category = item.Category, SubCategory = item.SubCategory, Date = item.Date, Staff = item.Staff, InvoiceTitle = item.InvoiceTitle, Amount = item.Amount, Remark = item.Remark, FileName = item.FileName };
            IsModalOpen = true;
        }
    }
}